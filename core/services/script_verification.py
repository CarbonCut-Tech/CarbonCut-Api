...
class ScriptVerificationService:
    TRACKING_SCRIPT_CDN = getattr(
        settings, 
        'TRACKING_SCRIPT_CDN', 
        'https://cdn.jsdelivr.net/gh/rishi-optiminastic/cc-cdn@main/dist/carboncut.min.js?v=2'
    )
    
    def verify_installation(self, url: str, api_key: str) -> Dict:
        try:
    ...
    def get_installation_instructions(self, api_key: str, domain: str) -> Dict:
        script_tag = f'''<script src="{self.TRACKING_SCRIPT_CDN}"></script>
<script>
  CarbonCutTracker.init({{
    ...
    autoTrack: true
        ...
        npm_command = "npm install @carboncut/tracker"
        installation_steps = [
